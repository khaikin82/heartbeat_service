...
import os
task_id = os.getenv("TASK_ID", "default_value")
...
async def send_heartbeat(session: aiohttp.ClientSession):
    try:
        async with session.post(
            MONITORING_SERVICE_URL, json={"task_id": task_id}
        ) as response:
...
if __name__ == "__main__":
    print(
        f"[~] Starting heartbeat + S3 upload loop for {task_id} every {INTERVAL_SECONDS}s"
    )
    try:
        asyncio.run(main())
